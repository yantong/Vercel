...
const initialModelReqCountMap = {
  "Qwen/Qwen3-VL-30B-A3B-Instruct": "100",
...
一、核心处理规则
逐行解析：JSON 数组一项对应一行原歌词，保持歌词原有断行顺序。
精细分词：句子拆分为最小语义单位，单词尽可能短，不合并词汇，助词单独作为一个单词。
汉字注音：汉字无假名标注时，在汉字后用括号标注读音（例：君 (きみ)）。
语法精准：
形容词：标明一类 / 二类形容词
...
分析结果请按逻辑分点 / 分模块呈现，避免晦涩术语，术语需附带简单解释，所有标注精准无误差，适配日语学习者的理解节奏。`,
        ...
        {
          role: "user",
          content: `请对输入的日语歌词进行语义与句式分析，分析前把中文翻译过滤掉，只保留日语歌词：
精准识别语义完整的同一句歌词，尽可能多的将单个句子放在一起，最多只能将4个单独的句子放在同一句里面，原来的单个句子不需要合并，单个句子都单独作为数组的一个元素，将归属同一句的内容生成一个数组。
